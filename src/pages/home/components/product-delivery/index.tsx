...
export default function ProductDelivery() {
  ...
  return (
    <div className="col-span-4 bg-white pt-6.25 pb-12 border border-[#EBEBEB] rounded-md mb-5">
      <div className="flex items-center justify-between px-6.25 mb-1.5">
        <h1 className="font-semibold text-[27px] text-[#484848]">Product Delivery</h1>
        <button className="btn">Order Details</button>
      </div>

      <div className="flex flex-col gap-3.5">
        {res.data.map((item) => (
          <div key={item.id} className="border-b border-[#EFEFEF] px-6.25 pt-2.75 pb-4">
            <div className="flex items-center gap-2.25 mb-1.75">
              <div>
                <img src={item.img} alt="item" className="w-14 h-13.75 object-cover object-center" />
              </div>
